...
class OrganizationCommunityAccessTest : AuthorizedControllerTest() {
  ...
  fun `community access does not open member endpoints`() {
    userAccount = testData.nonMember
    performAuthGet("/v2/organizations/${testData.otherOrg.id}/users").andIsNotFound
    performAuthGet("/v2/organizations/${testData.otherOrg.id}/usage").andIsNotFound
    performAuthGet("/v2/organizations/${testData.otherOrg.id}/invitations").andIsNotFound
    performAuthPut("/v2/organizations/${testData.otherOrg.id}/leave", null).andIsNotFound
  }
}
