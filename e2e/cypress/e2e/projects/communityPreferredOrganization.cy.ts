...
describe('Community preferred organization', () => {
  ...
  it('switches a community user to the owning organization and shows its public projects', () => {
    cy.intercept('GET', '**/v2/organizations/*/projects-with-stats*').as(
      'memberList'
    );
    openPublicProject();
    ...
    cy.visit(`${HOST}/`);
    waitForGlobalLoading();
    // 6 = only the switched org's public projects; "Community Outsider" (another org's public
    // project) must be filtered out, so a dropped organizationId filter fails this
    ...
    cy.get('@memberList.all').should('have.length', 0);
  });
  ...
  it('hides glossaries and translation memories in the foreign org settings', () => {
    openPublicProject();
    cy.visit(
